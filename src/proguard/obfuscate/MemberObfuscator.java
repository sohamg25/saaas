...
implements   MemberVisitor
{
	...
	public void visitAnyMember(Clazz clazz, Member member)
	{
		...
		if (newName == null)
		{
			...
			//nameFactory.reset();		//changed by Soham Ghosh
			nameFactory = new RandomNameFactory();		//added by Soham Ghosh

			if(!Configuration.forceUniqueClassMemberNames){		//added !Configuration.forceUniqueClassMemberNames by Soham Ghosh 
				do
				{
					newName = nameFactory.nextName(name);
					if(nameMap.containsKey(newName))
						nameFactory.reset();
					if(name.length()>3 && (name.substring(0, 3).equalsIgnoreCase("set") || name.substring(0, 3).equalsIgnoreCase("get"))){
